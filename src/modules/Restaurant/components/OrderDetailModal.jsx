...
export const OrderDetailModal = ({ show, onClose, orderId, onUpdateStatus, onReject, onRefund }) => {
  const [showRejectForm, setShowRejectForm] = useState(false);
  const [rejectReason, setRejectReason] = useState('');
  const [order , setOrders] = useState();
  const {orderDetail } = useOrder();

  const hanldeLoaddData = async () => {
            ...
            <div className="space-y-3">
              {order.orderDetails.map((item, idx) => (
                <div key={idx} className="flex items-center gap-4 p-3 bg-gray-50 rounded-lg">
                  <img src={item.imageId} alt={item.name} className="w-16 h-16 rounded-lg object-cover" />
                  ...
                  <p className="font-bold text-orange-600">{formatCurrency(item.totalPrice)}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="border-t pt-4">
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-gray-600">
                <span>Tạm tính ({order.numberDishes} món)</span>
                <span>{formatCurrency(order.totalAmount - order.deliveryFee)}</span>
              </div>
              <div className="flex justify-between text-gray-600">
                <span>Phí vận chuyển</span>
                <span>{order.deliveryFee === 0 ? 'Miễn phí' : formatCurrency(order.deliveryFee)}</span>
              </div>
